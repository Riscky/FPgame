...
timeHandler time w  = w >>= engineParticles >>= tparticles >>= tship >>= collisionHandler >>= tspawnEnemy >>= tspawnBonus >>= tenemies >>= tshoot >>= tbullets >>= tspawnStars >>= tmoveStars >>=updatetime
...
tship  time w@World{..} | dead == False = let (so,sl) = (shiporientation + ospeed,
...
tshoot    time w@World{..} = let bullets' =  if cond
                             ...
                             in w{bullets = bullets', spawnNextBullet =  if cond then 0.05 else spawnNextBullet - time}
                                    where cond = shootAction == Shoot && spawnNextBullet - time < 0 && dead == False -- Time based re-firing contr
